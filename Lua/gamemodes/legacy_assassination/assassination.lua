...
assassination.End = function ()
    ...
    for key, client in pairs(Client.ClientList) do
        local amount = math.floor(assassination.AwardedPoints[client.SteamID] or 0)
...
assassination.RenewAssassinationObjective = function(client, traitor, target)
    ...
    if not newObjective.Config.BaseAmountPoints then 
        newObjective.Config.BaseAmountPoints = newObjective.Config.AmountPoints
    end
    newObjective.Config.AmountPoints = newObjective.Config.BaseAmountPoints + objectivesCompleted * assassination.Config.PointsPerAssassination
    ...
    for _, objective in pairs(traitor.MainObjectives) do
        if not objective.EndRoundObjective and objective.IsCompleted() and not objective.Awarded then
            ...
            if client == nil then 
                ...
                Traitormod.SendObjectiveCompleted(client, objectiveText, points)
    ...
    for _, objective in pairs(traitor.SubObjectives) do
        if not objective.EndRoundObjective and objective.IsCompleted() and not objective.Awarded then
            ...
            if client == nil then 
                ...
                local points = Traitormod.AwardPoints(client, objective.Config.AmountPoints)
                Traitormod.SendObjectiveCompleted(client, objective.ObjectiveText, points)
            end
        end
    end
end
